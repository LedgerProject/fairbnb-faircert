            ...
            <div class="col py-2">
                <div class="row no-gutters justify-content-between align-items-center mb-2">
                    <div class="col-12 col-xl-auto flex-grow-1">
                        <!-- breadcrumbs -->
                        <ol id="breadcrumbs" class="breadcrumb text-size-md pt-1_5 pb-0_5 mb-0" itemscope=""
                            itemtype="http://schema.org/BreadcrumbList">
                            <li class="breadcrumb-item" itemprop="itemListElement" itemscope=""
                                itemtype="http://schema.org/ListItem">
                                <a href="/en/dashboard/" rel="" itemprop="item"><span itemprop="name"><i
                                                class="icon-bordered-home"></i></span></a>
                        ...
                        <div class="filter-toolbar">
                            <div class="input-group d-block d-md-flex flex-md-nowrap align-items-center">
                                ...
                                <div class="mb-0_5 mb-md-0">
                                    <a href="#" class="btn btn-light btn-sm px-1_5 filter-toolbar-btn view_profile_1s"
                    ...
                    <div @if (!$publicKey || $publicKey == "")style="display:none"@endif id="keypair_form" class="keypair col-md-8">
                        ...
                        <div class="form-group">
                            ...
                            {{-- ITC START --}}
                            <span id="private-key-missing-disclaimer" style="display: none; color:red;">Your private key has been deleted or you are using a different browser. Re-generate it!</span>
                            {{-- ITC END --}}
                        </div>
                        <button id="btn_regenerate" class="btn btn-primary" style="display: none;">Re-Generate Private key</button>
                    </div>
                    <div @if ($publicKey && $publicKey != "")style="display:none"@endif id="questions_form">

                        <h4>Local Ambassador Keypair Generation</h4>
                        {{-- ITC START --}}
                        <p id="question-form-disclaimer" style="color: #029fb5; margin-bottom: 30px;">Answer three questions and fill in your email address</p>
                        {{-- ITC END --}}
                        <div class="form-group">
                            <label for="ansInput1">What are the last five digit of your driving License number
                                ?</label>
                            <input type="text" class="form-control" id="ansInput1" name="ans_1"
                                   aria-describedby="emailHelp"
                                   placeholder="">
                        </div>
                        <div class="form-group">
                            <label for="ansInput2">In which town/city did your parents meet ?</label>
                            <input type="text" class="form-control" id="ansInput2" name="ans_2"
                                   aria-describedby="emailHelp"
                                   placeholder="">
                        </div>
                        <div class="form-group">
                            <label for="ansInput3">In which town/city was your first job ?</label>
                            <input type="text" class="form-control" id="ansInput3" name="ans_3"
                                   aria-describedby="emailHelp"
                                   placeholder="">
                        </div>
                        <div class="form-group">
                            <label for="ansInput4">What is your favourite movie ?</label>
                            <input type="text" class="form-control" id="ansInput4" name="ans_4"
                                   aria-describedby="emailHelp"
                                   placeholder="">
                        </div>
                        <div class="form-group">
                            <label for="ansInput5">What is the name of your favourite animal ?</label>
                            <input type="text" class="form-control" id="ansInput5" name="ans_5"
                                   aria-describedby="emailHelp"
                                   placeholder="">
                        </div>
                        <div class="form-group">
                            <label for="email">Email ?</label>
                            <input type="email" class="form-control" id="ans_email" name="ans_email"
                                   aria-describedby="emailHelp"
                                   placeholder="">
                        </div>

                        <button type="submit" onClick="encrypt()" id="btn_key_generate" class="btn btn-primary">Generate
                            Keypair
                        </button>
...
@push('scripts')
    ...
    <script type="module">
        ...
        $(document).on('click', '#btn_regenerate', function () {
            $('#keypair_form').css('display', 'none');
            $('#questions_form').css('display', 'block');


            $('#question-form-disclaimer').html('To re-generate your private key, you need to answer to questions in the same way you did during keypair creation!');
            $('#ansInput1').focus();
        });

        $(document).ready(function () {
        // ITC START
            // If public key is not stored on db, remove it (and private key) from local storage
            if(!publicKey || publicKey === '') {
                savePublicKeyOnLocalStorage(null);
                savePrivateKeyOnLocalStorage(null);
            } else {
                savePublicKeyOnLocalStorage(publicKey);
                setPublicKey();
            }

            var pk_exists = setPrivateKey();
            if(!pk_exists) {
                // Private key is missing but public key exists
                if(publicKey && publicKey !== '') {
                    $('#private-key-missing-disclaimer').css('display', 'block');
                    $('#btn_regenerate').css('display', 'block');
                }
                // Public/private key are missing
                else {
                    $('#keypair_form').css('display', 'none');
                    $('#questions_form').css('display', 'block');
                    $('#ansInput1').focus();
                    ...
                    zencode_exec(contract, {
                        ...
                        axios.post(" {{ route('ledger.keypair.answers') }} ", {
                          ...
                          Then print my data`;
                                ...
                                zencode_exec(contract2, {key: ``, data: mySeedData, conf: ``}).then((resultKeyPair) => {
                                    ...
                                    axios.post(" {{ route('ledger.save.publickey') }} ", {
                                        ...
                                        if (resultSaveOnDb.data === 1) {
                                            ...
                                            publicKey = parsed_result[id]['keypair']['public_key'];
                                            saveKeysOnLocalStorage(id, publicKey, parsed_result[id]['keypair']['private_key']);

                                            //Fill the form with the keys
                                            setPublicKey();
                                            setPrivateKey();

                                            // Hide private key warning and re-generate btn
                                            $('#private-key-missing-disclaimer').css('display', 'none');
                                            $('#btn_regenerate').css('display', 'none');

                                            // Hide questions
                                            $('#questions_form').css('display', 'none');

                                            // Show keypair form
                                            $('#keypair_form').css('display', 'block');

                                            //show success message
                                            sweetAlert('success', "Your keys are successfully created!", "itc_message_confirm");
                                        } else {
                                ...
                                $('.pagination-loader').addClass('d-none');
                                sweetAlert('error', "Your answers are wrong. Try again!");
                            }
...
//console.log(parsed_result);
        ...
        function saveKeysOnLocalStorage(id, public_key, private_key) {
            ...
            savePublicKeyOnLocalStorage(public_key);
            savePrivateKeyOnLocalStorage(private_key);
        }

        function savePublicKeyOnLocalStorage(public_key) {
        ...
        function savePrivateKeyOnLocalStorage(private_key) {
